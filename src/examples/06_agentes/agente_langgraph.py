...
# Configurar el modelo del agente que soporte tool calling
model = init_chat_model(model="openai:gpt-4o-mini", temperature=0)

# Prompt del agente que define su comportamiento
system_prompt = """Eres un asistente de email profesional. Para procesar emails sigue EXACTAMENTE estos pasos:

    1. PRIMERO: Usa 'search_gmail' con query 'in:inbox' para obtener la lista de mensajes en la bandeja de entrada PRINCIPAL.
    ...
    6. SEXTO: Usa 'create_gmail_reply_draft' para crear un borrador de RESPUESTA (no email nuevo) con:
    ...
    - SIEMPRE incluye "thread_id" en create_gmail_reply_draft para que sea una respuesta, no un email nuevo
        ...
        response = agent_executor.invoke(
            {
                "messages": [
                    (
                        "human",
                        "Procesa el email más reciente en la bandeja de entrada y genera un borrador de respuesta profesional.",
                    )
                ]
            },
            config=config,
