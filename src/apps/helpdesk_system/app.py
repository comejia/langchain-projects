...
import uuid
from graph import create_helpdesk, HelpdeskState
from setup_rag import DocumentProcessor
from datetime import datetime
import traceback

# Configuración de página
st.set_page_config(page_title="Helpdesk 2.0 con RAG", page_icon="🎧", layout="wide")
...
def procesar_consulta(consulta: str, ticket_id: str):
    ...
    # Procesar con streaming (observabilidad)
    historial_procesamiento = []

    try:
        for chunk in st.session_state.helpdesk.stream(
            estado_inicial, config=config, stream_mode="updates"
        ):
            ...
            for ticket_id, ticket_data in reversed(
                ...
                with st.expander(
                    ...
                    if resultado.get("requiere_humano") and not resultado.get(
                        ...
                        with col_btn1:
                            if st.button("💾 Enviar Respuesta", key=f"btn_{ticket_id}"):
