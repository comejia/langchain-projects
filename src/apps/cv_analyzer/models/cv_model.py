...
class CVAnalyzer(BaseModel):
    ...
    skills: list[str] = Field(
        description="Lista de las 5-7 habilidades más relevantes para el puesto."
    )
    education: str = Field(
        description="Nivel educativo más alto y especialización principal."
    )
    experience_key: str = Field(
        description="Resumen conciso de la experiencia más relevante para el puesto especifico."
    )
    strengths: list[str] = Field(
        description="3-5 principales fortalezas del candidato basadas en su perfil."
    )
    weaknesses: list[str] = Field(
        description="2-4 areas donde el candidato podría desarrollarse o mejorar."
    )
